...
class TestTodoApi():
    ...
    def test_post_response_201_status(self, api_auth_jwt_client):
        ...
        """
        response = api_auth_jwt_client.post(reverse('todo-list'), {
            'title': 'tests',
            'status': 'false',
        })
    ...
    def test_post_response_401_status(self, api_anonymous_client):
        ...
        """
        response = api_anonymous_client.post(reverse('todo-list'), {
            'title': 'tests',
            'status': 'false',
        })
        assert response.status_code == 401
    ...
    def test_pk_retrieve_response_200_status(self, api_auth_jwt_client, get_last_object_pk):
        """
        for testing retrieving todo api with authenticated user
        """
        response = api_auth_jwt_client.get(reverse('todo-detail', kwargs={'pk': get_last_object_pk}))
        assert response.status_code == 200

    def test_pk_update_put_response_200_status(self, api_auth_jwt_client, get_last_object_pk):
        """
    ...
    def test_pk_delete_response_204_status(self, api_auth_jwt_client, get_last_object_pk):
        """
